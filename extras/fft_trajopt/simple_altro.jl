...
function backward_pass!(params,X,U,P,p,d,K,reg,μ,μx,ρ,λ)
    ...
    # hxv = X[N][params.term_idx] - params.Xref[N][params.term_idx]
    # # ∇hx = diagm(ones(params.nx))
    # ∇hx = zeros(length(params.term_idx), params.nx)
    # for i = 1:length(params.term_idx)
    #     ∇hx[i,params.term_idx[i]] = 1
    # end
    hxv = term_con(X[N], params)
    ∇hx = term_con_jac(params)
...
function term_con(x,params)
    x[params.term_idx] - params.Xref[params.N][params.term_idx]
end
function term_con_jac(params)
    ∇hx = zeros(length(params.term_idx), params.nx)
    for i = 1:length(params.term_idx)
        ∇hx[i,params.term_idx[i]] = 1
    end
