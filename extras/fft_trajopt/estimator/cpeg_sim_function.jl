...
function run_cpeg_sim(path;verbose = false)
    ...
    # get atmosphere stuff in params
    altitudes,densities, Ewind, Nwind = load_atmo(;path)
    nx = 7
    nu = 2
    ...
    Xsim = X
    Usim = U
    alt, dr, cr, σ, r, v = process_ev_run_2(ev,Xsim,Usim)
    alt_g, dr_g, cr_g = cp.postprocess_scaled(ev,[SVector{7}(params.x_desired)],SVector{7}(Xsim[1]))
    t_vec = (0:(length(X)-1))*sim_dt

    dr_error = dr[end] - dr_g[1]
    cr_error = cr[end] - cr_g[1]

    σ̇ = 1*U
    return alt, dr, cr, σ, t_vec, σ̇, dr_error, cr_error, qp_iters, alt_g, dr_g, cr_g
end
