...
function density_spline(p::CPEGDensityParameters, h::T) where T
   ...
   mean_coeff_a = [-1.85730963217633e-06
                    ...
                    0]
    mean_coeff_b = [3.54484471476105e-05
                    ...
                    0]
    mean_coeff_c = [-0.000551292656877884
                    ...
                    0]
    mean_coeff_d = [0.00570116219576596
                    0.00359874638604477
                    0.00217839331286464
                    0.00137429307852056
    ...
    breaks = [10:5:60; 62:2.:70; 71:1.:121]
    # show(breaks)
    for jj in range(1, length(breaks)-1)
        if (h < breaks[jj+1] && h >= breaks[jj])
            # show(h)
            return mean_coeff_d[jj] + mean_coeff_c[jj]*(h-breaks[jj]) +mean_coeff_b[jj]*(h-breaks[jj])^2 +mean_coeff_a[jj]*(h-breaks[jj])^3
        end
    end
end
